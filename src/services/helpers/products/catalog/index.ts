...
export const getAllCatalogs = async () => {
    return await catalog.find({ $or: [{ name: 'Electronics' },{ name: 'Phones & Tablets' }, { name: 'Home,Appliances & furniture' }] }, '-sub_categories')
}
...
export const getCatelogById = async (id: string) => {
    ...
    if(!cat)
        throw new createError.BadRequest('No catalog with this Id')
    return cat
}
